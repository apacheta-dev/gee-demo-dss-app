...
exports.createColorRampLegendPanel = function (pTitle, pVis, pTextMin, pTextCenter, pTextMax) {
    ...
    // Allow the use of labels instead of min/max values 
...
exports.createLayerEntry = function (layer) {
    ...
    // If 'singleColor' show reference color besides text
    var colorEntry = ui.Label();

    if (layer.singleColor) {
        var color = layer.style.color !== undefined ? layer.style.color : layer.style.palette[0];
        var fillColor = layer.style.fillColor !== undefined ? layer.style.fillColor : color;
    ...
    var pnlColor = ui.Panel(colorEntry);
    pnlColor.style().set({ width: '10%' });

    pnlLayerEntry.widgets().add(pnlColor);

    var pnlCitation = ui.Panel();
    pnlCitation.style().set({ width: '10%' });
    if (layer.citation !== undefined && layer.citation !== '') {
        // Add 'i' icon ui.Label with link to document 
        var lblCitation = ui.Label('ⓘ', { margin: "0 2px 2px 2px", color: 'blue' }, layer.citation);
        pnlCitation.add(lblCitation);

    }

    pnlLayerEntry.widgets().add(pnlCitation);

    // Add opacity control
    var sldOpacity = ui.Slider({
        min: 0,
        max: 1,
        value: 1,
        step: 0.1, style: { margin: '4px 0px', width: '10%', fontSize: '12px', backgroundColor: 'white' },
    });


    pnlLayerEntry.widgets().add(sldOpacity);
